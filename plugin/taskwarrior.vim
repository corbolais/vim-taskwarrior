...
let g:task_all_commands             = split(system('/usr/bin/task _command'), '\n')
let g:task_all_configurations       = split(system('/usr/bin/task _config'), '\n')
...
" let g:task_log_directory = get(g:, 'task_log_file', system('/usr/bin/task _get -- rc.data.location')[0:-2])
let g:task_log_directory            = get(g:, 'task_log_file', matchstr(system('/usr/bin/task show | grep data.location')[0:-2], '\S*$'))
